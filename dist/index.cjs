...
class ByteArrayConverter {
    ...
    static encodeByteArrayToString(byteArray, radix) {
        ...
        if (!Number.isInteger(radix))
            throw new TypeError('radix is not an integer');
        if ((radix < 2 || radix > 36) && radix !== 64)
            throw new RangeError('radix is not between 2 and 36 or 64');
    ...
    static decodeStringToByteArray(encodedString, radix) {
        if (typeof encodedString !== 'string')
            throw new TypeError('encodedString is not a string');
        if (!Number.isInteger(radix))
            throw new TypeError('radix is not an integer');
        if ((radix < 2 || radix > 36) && radix !== 64)
            throw new RangeError('radix is not between 2 and 36 or 64');
        if (radix === 64) {
            if (typeof atob === 'undefined')
                return Uint8Array.from(Buffer.from(encodedString, 'base64'));
...
class StorageManager {
    ...
    constructor(itemName, options = {}) {
        const { defaultValue, encodeFn = (value) => StorageManager.TRA.encrypt(value, 64), decodeFn = (value) => StorageManager.TRA.decrypt(value, 64), storage = window.localStorage, } = options;
    ...
    sync(decodeFn = this.decodeFn) {
        ...
        if (!value.startsWith('\0JSON\0\x20'))
            return (this.value = value); // value can only be of type T as it is checked on assignment
        // Slice off '\0JSON\0\x20' prefix
        value = value.slice(7);
        // Manually convert unparseable JSON object
        if (value === 'undefined')
            return (this.value = undefined); // this can only ever happen if type T allows undefined
